...
export async function getUserTypeResources(userId : string, resourceType:string) : Promise<string[]> {
    const functionName = "getUserTypeResources";
    ...
    try {
        ...
        const resources = await prisma.sgs_user_resource.findMany({
            where: {
                user_id: userId,
                status: 'A',
                type_resource : resourceType.toUpperCase()
            }
        });
        resources.forEach(resource =>{
            userResources.push(resource.resource_id);
        });
