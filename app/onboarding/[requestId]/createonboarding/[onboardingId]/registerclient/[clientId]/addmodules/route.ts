...
export async function POST(request:NextRequest, { params }: { params: Promise<{requestId:string, onboardingId: string, clientId:string }> }) {
    try {
        ...
        const addNewClientModules = await addNewClientBaseModules(requestId,onboardingId, clientId);
        if (addNewClientModules.includes("ERROR")) return NextResponse.json({addBaseModulesErrorMessage : addNewClientModules}, { status: 400 });
