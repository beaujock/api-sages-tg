import { registerNewClient} from "@/factories/ONBOARDING/onboardingFactory";
...
export async function POST(request:NextRequest, { params }: { params: Promise<{requestId:string, onboardingId: string }> }) {
    try {
        ...
        logError('F',"Nouveau client non enregistré",(new URL(request.url)).pathname, error.message, true);
