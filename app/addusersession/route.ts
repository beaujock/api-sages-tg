...
export async function POST(request:NextRequest) {
    try {
        ...
        const sessionAdded:boolean = await addUserSession(decodedToken.user_id, sessionRequest.token, new Date(sessionRequest.token_effective_time), new Date(sessionRequest.token_expiry_time));
