...
import { getAllResourceTypes } from "@/factories/ALL_USAGE/SagesTgFactory";
...
export async function GET(request:NextRequest, { params }: { params: Promise<{clientCode: string}> }) {
    try {
        ...
        return NextResponse.json({resourceTypes : resourceTypes}, { status: 200 });
    }
    catch(error:any) {
        logError('F',"Echec : Retrouver tous les types de resource",(new URL(request.url)).pathname, error.message, true);
