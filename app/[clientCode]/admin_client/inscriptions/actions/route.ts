...
export async function GET(request:NextRequest, { params }: { params: Promise<{clientCode: string}> }) {
    try {
        ...
        const actions = await getClientMenuItemActions(client.id, role.id, "INSCRIPTIONS");
        return NextResponse.json({actions: actions}, { status: 200 });
    }
    catch(error:any) {
        logError('F',"Echec : Actions (inscriptions)",(new URL(request.url)).pathname, error.message, true);
